...
bool PlayGameScene::init()
{
	...
	downItem->setPosition(Vec2(button->getPosition().x, button->getPosition().y - button->getContentSize().height * 0.2 / 4));
...
void PlayGameScene::addAt(int x, int y, int type)
{
	...
	case 1:
		...
		monsters[numOfMonster]->getSprite()->setPosition(x, y);
		//auto enemyBody = PhysicsBody::createBox(enemy->getContentSize());
		//enemy->setPhysicsBody(enemyBody);
		this->addChild(monsters[numOfMonster]->getSprite());
		numOfMonster++;
...
void PlayGameScene::attackBoss(float dt)
{
	boss->attack(dt);
}